...
type OrchestratedServiceManager interface {
	...
	CreateNewNamespace(ctx context.Context, namespace string, labels []model.KeyValue) (*model.BaseResource, error)
	...
	UpdateService(ctx context.Context, namespace, serviceName string, port int32) (*model.Service, error)
	//gracePeriod in seconds, if 0 it will be deleted immediately, if negative it will be deleted after the default grace period
	...
	UpdateConfigMap(ctx context.Context, namespace, configMapName string, data []model.KeyValue) (*model.ConfigMap, error)
	//gracePeriod in seconds, if 0 it will be deleted immediately, if negative it will be deleted after the default grace period
